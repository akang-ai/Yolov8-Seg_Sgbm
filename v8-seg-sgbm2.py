...
yolo_model = YOLO('yolov8s-seg.pt').to(device)  # 使用分割模型并移至GPU

# 相机参数（根据实际标定调整）
focal_length = 427.0117
...
def process_frame_async(frame1, frame2, is_video=False):
    ...
    with ThreadPoolExecutor(max_workers=2) as executor:
        ...
        # 提交YOLO任务
        yolo_future = executor.submit(yolo_inference_worker, frame1, yolo_queue)
    ...
    return result_frame, disp_color
    ...
    if mode == "predict":
        while True:
            ...
            try:
                ...
                result_frame, disp_color = process_frame_async(frame1.copy(), frame2.copy())
        ...
        if video_save_path != "":
            ...
            out = cv2.VideoWriter(video_save_path, fourcc, video_fps, (frame_width // 2, frame_height))
        ...
        for _ in range(5):
            ...
            _, _ = process_frame_async(frame1.copy(), frame2.copy(), is_video=True)
        ...
        while True:
            ...
            result_frame, disp_color = process_frame_async(frame1.copy(), frame2.copy(), is_video=True)
        ...
        if image is None:
            ...
            print("预热模型...")
            for _ in range(5):
                _, _ = process_frame_async(frame1.copy(), frame2.copy())
        ...
        for img_name in tqdm(img_names):
            ...
            # 处理帧
            start_time = time.time()
            result_frame, _ = process_frame_async(frame1.copy(), frame2.copy())
            process_time = time.time() - start_time
